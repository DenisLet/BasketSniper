...
def take_it_lower3(line):
    normalize, qwt_1, qwt_2, values_list  = range(40,45), sum(line[:2]), sum(line[2:4]), list()
    for value in normalize:
        if qwt_1 > value and qwt_2 > value:
            print('TWO QWT MORE OR EQUAL:: ', value)
            values_list.append(value)

    return max(values_list) if len(values_list) != 0 else False
    ...
    try:
        ...
        for i in matches:
            try:
                ...
                if period == 3 and ((current_third < 8 and minute >=4) or (current_third<11 and minute>=5)
                    or (current_score < 15 and minute >=6)):
                    link3 = (get_link(i), minute)
                    print("3"*50)
                    if link3 in period3_list:
                        continue
                    period3_list.add(link3)
                    checker = 3
                    check_link(link3[0], time, score_line[4], score_line[5], period, minute, checker, score_line)
                    continue

                if period == 40 and ((current_fourth < 8 and minute >=4) or (current_fourth<11 and minute>=5)
                    or (current_score < 15 and minute >=6)):
                    link4 = (get_link(i), minute)
                    print("4"*50)
                    if link4 in period4_list:
                        continue
                    period4_list.add(link4)
                    checker = 4
                    check_link(link4[0], time, score_line[6], score_line[7], period, minute, checker, score_line)
                    continue

                '''    winstreak & totalsteak  '''

                if period == "HT" or period == 4 or (period == 3 and minute == 10):
                    winstreak_ = try_it_out(score_line)

                    totalsteak_down = take_it_lower3(score_line)
                ...
                print(fail)
                print("SOME BULLSHIT IN PARSING")
                continue
